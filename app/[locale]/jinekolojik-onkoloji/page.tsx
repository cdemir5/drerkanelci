...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const category = getCategoryByLocale(locale, "jinekolojik-onkoloji")!;
  return { title: category.metaTitle, description: category.metaDescription };
}

export default async function JinekolojikOnkolojiPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
